...
public class ReadXMLFile {
  ...
  public static void main(String mode) {
    ...
    try {
    	...
    	File logFile = new File("txtxml/testxml.txt");
    	writer = new BufferedWriter(new FileWriter(logFile));
    	File fXmlFile = null;
    	
    	//create a filepath to the classcycleAnalyse.xml file
		if (mode.equals("JPacman")){
			fXmlFile = new File("txtxml/ClassycleAnalyse.xml");
		}else if (mode.equals("JOD")){
			fXmlFile = new File("txtxml/jod_ClassycleAnalyse.xml");
		}else{
			
			System.out.println("error opening XML file for Classycle");
			return;
		}
